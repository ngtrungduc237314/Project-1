...
# =========================
# 3. BINNING FEATURE (Valley-based cut)
# =========================
# Các "thung lũng" quan sát từ phân phối KDE
bins = [-np.inf, 2.7, 3.7, 4.7, 5.7, np.inf]
labels = [1, 2, 3, 4, 5]

data['bedrooms_binned'] = pd.cut(
    data['Avg. Area Number of Bedrooms'],
    bins=bins,
    labels=labels
)

# ép về số (model dễ học hơn)
data['bedrooms_binned'] = data['bedrooms_binned'].astype(int)

# drop biến gốc
data = data.drop('Avg. Area Number of Bedrooms', axis=1)

print("\nBinning result sample:")
print(data[['bedrooms_binned']].head())
...
plt.xlabel("Actual Price (x100k)")
plt.ylabel("Predicted Price (x100k)")
...
plt.xlabel("Predicted Price (x100k)")
