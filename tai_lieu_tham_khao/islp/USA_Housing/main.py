...
# 1. LOAD DATA
# =========================
USA_Housing = pd.read_csv('USA_Housing.csv', usecols=lambda col: col != 'Address')
USA_Housing = USA_Housing.dropna()

# Fix tên cột nếu có lỗi format
USA_Housing.columns = USA_Housing.columns.str.strip().str.lower()

print("Shape:", USA_Housing.shape)

# =========================
# 2. TẠO X, Y
# =========================
design = MS(USA_Housing.columns.drop('price')).fit(USA_Housing)

Y = np.array(USA_Housing['price'])
X = design.transform(USA_Housing)
n = len(Y)
...
full_path.fit(USA_Housing, Y)

# =========================
# 4. IN-SAMPLE
# =========================
Yhat_in = full_path.predict(USA_Housing)
...
# 1. Làm đường đen dày cộp lên (linewidth=5)
ax.plot(np.arange(n_steps), insample_mse, 'k', linewidth=5, label='In-sample')

# 2. Biến đường đỏ thành nét đứt (linestyle='--')
ax.errorbar(
    np.arange(n_steps),
    cv_mean,
    cv_se,
    color='r',
    linestyle='--', # Thêm dòng này
    linewidth=2,
